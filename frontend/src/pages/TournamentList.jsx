...
export default function TournamentList() {
  ...
  const load = async () => {
    try {
      ...
      setError(
        e?.response?.data?.message || e?.message || "Failed to load tournaments"
      );
      setItems([]);
    } finally {
      setLoading(false);
  ...
  const games = useMemo(() => {
    const gset = new Set(
      (items || []).map((t) => (t.game || "").trim()).filter(Boolean)
    );
  ...
  const filtered = useMemo(() => {
        ...
        const orgName =
          typeof t.organizationId === "object"
            ? (
                ...
                t.organizationId?.organizationInfo?.orgName ||
                ""
              ).toLowerCase()
            : "";
        return (
          title.includes(term) || g.includes(term) || orgName.includes(term)
        );
      });
    }
    ...
    arr.sort((a, b) => {
      const aDate = new Date(
        a.startAt || a.timeSlot?.start || a.date || 0
      ).getTime();
      const bDate = new Date(
        b.startAt || b.timeSlot?.start || b.date || 0
      ).getTime();
  ...
  return (
    <>
      <SEO
        title="Esports Tournaments – Browse & Register | ArenaPulse"
        description="Discover and join upcoming esports tournaments hosted by verified organizations. ArenaPulse brings all tournaments under one roof — no more Discord chaos."
        keywords="esports tournaments, join tournaments, gaming competitions, host events"
        canonical="https://thearenapulse.xyz/tournaments"
        schema={{
          "@context": "https://schema.org",
          "@type": "CollectionPage",
          name: "Tournament List",
          description: "Explore upcoming esports tournaments on ArenaPulse.",
        }}
      />
      ...
      <div className="mx-auto max-w-7xl p-6">
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-white/60">
              Find, filter & join your next competition.
            </p>
          </div>
          <Link
            to="/tournaments/new"
            className="inline-flex items-center justify-center rounded-xl bg-indigo-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-600 active:scale-[0.99]"
          >
            Create Tournament
          </Link>
        </div>
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          ...
          <div className="relative">
            ...
            >
              <option className="bg-gray-900" value="date_desc">
                Newest first
              </option>
              <option className="bg-gray-900" value="fee_asc">
                Lowest fee
              </option>
              <option className="bg-gray-900" value="fee_desc">
                Highest fee
              </option>
              <option className="bg-gray-900" value="cap_desc">
                Highest capacity
              </option>
            </select>
            <ChevronRight className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 rotate-90 text-white/50" />
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute left-3 top-3 flex items-center gap-2">
                      <StatusBadge
                        startAt={t.startAt || t?.timeSlot?.start}
                        endAt={t.endAt || t?.timeSlot?.end}
                      />
