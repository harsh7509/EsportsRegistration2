...
import {
  Gamepad2, Mail, Lock, User, Building, ShieldCheck, Loader2,
  Eye, EyeOff, AlertCircle, CheckCircle2, TimerReset
} from 'lucide-react';
...
const strengthMeta = (pwd = '') => {
  ...
  const colors = ['#ef4444','#f97316','#f59e0b','#22c55e','#16a34a','#0ea5e9'];
  return {
    score,
...
const RoleCard = ({ active, icon: Icon, title, desc, onClick }) => (
  ...
  >
    <div className={`grid h-10 w-10 place-items-center rounded-lg ${active ? 'bg-indigo-500/20' : 'bg-white/10'}`}>
      <Icon className={`${active ? 'text-indigo-300' : 'text-white/80'} h-5 w-5`} />
    </div>
    <div>
...
const Signup = () => {
  ...
  const validation = useMemo(() => {
    const e = {};
    if (!form.name.trim()) e.name = form.role === 'organization' ? 'Organization name is required' : 'Name is required';
    ...
    <div className="min-h-screen relative flex items-center justify-center p-6">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="mb-6 text-center">
          ...
          <h2 className="mt-4 text-3xl font-bold tracking-tight">Join the Competition</h2>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
          {!otpStep ? (
            <form className="space-y-6" onSubmit={handleSubmit} noValidate>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <RoleCard
                    ...
                    onClick={() => setForm((s) => ({ ...s, role: 'organization' }))}
                  />
                </div>
              </div>
              ...
              {/* name */}
              <div>
                <label htmlFor="name" className="mb-1.5 block text-xs text-white/70">
                  {form.role === 'organization' ? 'Organization name' : 'Your name'}
                </label>
                <div className="relative">
                  <User className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/50" />
                  <input
                    id="name" name="name" type="text"
                    className={`w-full rounded-xl border bg-white/5 px-10 py-2.75 text-sm text-white outline-none placeholder:text-white/40 focus:border-white/20 focus:ring-2 focus:ring-indigo-500/30 border-white/10 ${errors.name ? 'ring-2 ring-rose-500/40 border-rose-500/40':''}`}
                    placeholder={form.role === 'organization' ? 'e.g., TechOrg' : 'e.g., Ayaan Sharma'}
                    value={form.name} onChange={onChange}
                  />
                </div>
                {errors.name && <p className="mt-1.5 flex items-center gap-1 text-xs text-rose-300"><AlertCircle className="h-3.5 w-3.5" />{errors.name}</p>}
              </div>

              {/* email */}
              <div>
                <label htmlFor="email" className="mb-1.5 block text-xs text-white/70">Email</label>
                <div className="relative">
                  <Mail className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/50" />
                  <input
                    id="email" name="email" type="email" autoComplete="email"
                    className={`w-full rounded-xl border bg-white/5 px-10 py-2.75 text-sm text-white outline-none placeholder:text-white/40 focus:border-white/20 focus:ring-2 focus:ring-indigo-500/30 border-white/10 ${errors.email ? 'ring-2 ring-rose-500/40 border-rose-500/40':''}`}
                    placeholder="you@example.com"
                    value={form.email} onChange={onChange}
                  />
                </div>
                {errors.email && <p className="mt-1.5 flex items-center gap-1 text-xs text-rose-300"><AlertCircle className="h-3.5 w-3.5" />{errors.email}</p>}
              </div>
              ...
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-white/50">min. 6 chars</span>
                </div>
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full rounded-xl border bg-white/5 px-10 py-2.75 text-sm text-white outline-none placeholder:text-white/40 focus:border-white/20 focus:ring-2 focus:ring-indigo-500/30 border-white/10 ${errors.password ? 'ring-2 ring-rose-500/40 border-rose-500/40':''}`}
              ...
              >
                <span className="inline-flex items-center justify-center gap-2">
                  {submitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <CheckCircle2 className="h-5 w-5 opacity-80" />}
