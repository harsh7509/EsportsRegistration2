...
export default function OrganizationProfile() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        let list = res?.data?.items || res?.data?.tournaments || res?.data || [];

        // fallback: fetch all and filter client-side
        if (!Array.isArray(list) || !list.length) {
          const all = await tournamentsAPI.list();
          const arr = all?.data?.items || all?.data?.tournaments || all?.data || [];
          list = arr.filter((t) => {
  ...
  const banner =
    organization?.organizationInfo?.coverUrl || organization?.coverImage || null;
  ...
  const handleUploadAvatar = async (e) => {
    ...
    try {
      ...
      setImgBroken(false);
      setOrgData((prev) =>
        prev ? { ...prev, organization: { ...prev.organization, avatarUrl: imageUrl } } : prev
      );
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="text-center">
          <h2 className="mb-2 text-2xl font-bold text-rose-400">Organization Not Found</h2>
          <p className="text-slate-400">Please check the URL or try again later.</p>
        </div>
      </div>
    );
  }
              ...
              <div className="min-w-0 flex-1">
                ...
                {organization.organizationInfo?.location && (
                  <div className="mb-3 inline-flex items-center gap-2 text-slate-300">
                    ...
                    <span className="truncate">{organization.organizationInfo.location}</span>
                  </div>
                )}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    Rating breakdown {showBreakdown ? <ChevronUp className="inline h-3 w-3" /> : <ChevronDown className="inline h-3 w-3" />}
                  </button>
                  ...
                  <button
                    ...
                    className="text-xs px-2 py-1 rounded-lg bg-white/10 hover:bg-white/15 border border-white/15"
                  >
                    Reviews {showReviews ? <ChevronUp className="inline h-3 w-3" /> : <ChevronDown className="inline h-3 w-3" />}
                  </button>
                </div>

                {uploading && <div className="mt-2 text-sm text-slate-300">Uploading photo…</div>}
              </div>

              {/* Stats */}
              <div className="hidden text-right sm:block">
                <div className="text-3xl font-bold text-indigo-400">{scrims?.length || 0}</div>
          ...
          <div className="space-y-6 lg:col-span-1">
            ...
            {showBreakdown && (
              <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-6 ring-1 ring-white/10">
                <h2 className="mb-4 text-xl font-semibold text-white">Rating Breakdown</h2>
                {orgData && Object.keys(categoryAverages || {}).length ? (
                  ...
                  <p className="text-sm text-slate-400">No category ratings yet.</p>
                )}
              </div>
            )}
                ...
                <div className="space-y-4">
                  {ratings?.length ? (
                    ratings.slice(0, 6).map((r) => (
                      <div key={r._id} className="rounded-xl bg-white/5 p-4 ring-1 ring-white/10">
                        ...
                        {r.comment && (
                          <p className="mb-2 text-sm text-slate-200">{r.comment}</p>
                        )}
                        {r.scrimId?.title && (
                          <div className="text-xs text-slate-400">for “{r.scrimId.title}”</div>
                        )}
                      </div>
                    ))
                  ) : (
                    <p className="py-4 text-center text-slate-400">No reviews yet</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-6 ring-1 ring-white/10">
              <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="hidden text-right sm:block">
                    ...
                    <div className="text-2xl font-bold text-indigo-400">{scrims?.length || 0}</div>
                  </div>
                  {scrims?.length > 4 && (
                    <button
              ...
              {visibleScrims?.length ? (
                ...
                <div className="py-12 text-center">
                  ...
                  <h3 className="mb-1 text-xl font-medium text-slate-300">No scrims yet</h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {past?.length > 0 && (
                    <button
                      ...
                      className="text-xs px-2 py-1 rounded-lg bg-white/10 hover:bg-white/15 border border-white/15"
                    >
                      {showPastTournaments ? 'Hide past' : 'See past tournaments'}
                    </button>
                  )}
                </div>
              </div>
              ...
              {tourLoading ? (
                ...
                <>
                  ...
                  {showPastTournaments && past?.length > 0 && (
                    <div className="mt-8">
                      <h3 className="mb-3 text-sm font-medium text-slate-300">Past</h3>
                      <div className="grid gap-6 md:grid-cols-2">
                        {past.map((t) => (
                          <TournamentCard key={t._id} t={t} />
                        ))}
  ...
  return (
    <div className="rounded-xl bg-white/5 p-4 ring-1 ring-white/10 hover:ring-white/20 transition">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold text-white line-clamp-1">{t?.title || 'Tournament'}</h3>
      ...
      <div className="mt-3 flex items-center justify-between">
        <div className="text-xs text-slate-400">Capacity: {t?.capacity || 0}</div>
        <Link
