...
const PlayerCard = ({ index, value, onChange, required }) => (
  <div className="rounded-xl border border-white/10 bg-white/5 p-3">
    <div className="mb-2 flex items-center justify-between text-xs">
      ...
      <div className={`text-[10px] ${required ? 'text-rose-300/90' : 'text-white/50'}`}>
...
const PrizePool = ({ tournament, compact = false }) => {
  ...
  } else if (tournament?.prizeBreakdown && typeof tournament.prizeBreakdown === 'object') {
    breakdown = Object.entries(tournament.prizeBreakdown).map(([place, amount]) => ({
      place: Number(place),
              ...
              .map((row, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2 text-sm">
                  <span className="text-white/80">
                    {row.place === 1 ? '1st' : row.place === 2 ? '2nd' : row.place === 3 ? '3rd' : `${row.place}th`} Place
                  </span>
                  <span className="font-semibold">{fmt(Number(row.amount) || 0)}</span>
                </div>
              ))}
          </div>
        )}
...
const TournamentDetails = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await (tournamentsAPI.getDetails ? tournamentsAPI.getDetails(id) : tournamentsAPI.get(id));
  ...
  const dateStr = useMemo(() => (dateValue ? new Date(dateValue).toLocaleString() : 'TBA'), [dateValue]);

  // Normalize organizer object
  const org = typeof t?.organizationId === 'object' ? t.organizationId : (t?.organizationId ? { _id: t.organizationId } : null);

  const capacity = Math.max(0, Number(t?.capacity || 0));
  const registeredCount = Math.min(capacity || 0, Number(t?.registeredCount || 0));
  const filledPct = capacity ? Math.round((registeredCount / capacity) * 100) : 0;

  const entryLabel = Number(t?.entryFee || 0) > 0 ? `₹${Number(t.entryFee).toLocaleString('en-IN')}` : 'Free';

  const canEdit = useMemo(() => {
    if (!user || !t) return false;
    if (user.role === 'admin') return true;
    const uid = user._id || user.id;
    const orgId = org?._id || (typeof t?.organizationId === 'string' ? t.organizationId : null);
    const createdById =
      (typeof t?.createdBy === 'object' && t.createdBy?._id) ||
      (typeof t?.createdBy === 'string' ? t.createdBy : null);
    return user.role === 'organization' && (uid && (uid === orgId || uid === createdById));
  }, [user, t, org]);
  ...
  const isRegistered = useMemo(() => {
    ...
    const inRegistrations = Array.isArray(t.registrations)
      ? t.registrations.some(r =>
          (r?.userId?._id || r?.captainId || r?.teamLeadId || r?.userId || r?._id) === uid
        )
      : false;
    ...
    <div className="relative">
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-white/10 bg-gray-900/70 backdrop-blur supports-[backdrop-filter]:bg-gray-900/50 p-3 sm:hidden">
        <div className="mx-auto flex max-w-5xl items-center gap-2">
          {t.registrationUrl ? (
            ...
            >
              {isRegistered ? 'Already Registered' : (showRegForm ? 'Hide Form' : 'Register Now')}
            </button>
          )}
      ...
      <div className="max-w-5xl mx-auto p-6 pb-24 sm:pb-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          <Link to="/tournaments" className="inline-flex items-center gap-2 text-white/80 hover:text-white">
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-gray-800/60 to-gray-900/60 backdrop-blur">
          <div className="relative">
            ...
            <div className="absolute bottom-4 left-4 right-4 flex flex-wrap items-center gap-2">
              ...
              {capacity > 0 && <Badge>{registeredCount}/{capacity} slots</Badge>}
              <Badge className="hidden sm:inline-flex"><ShieldCheck className="mr-1.5 h-3.5 w-3.5" /> Verified Tournament</Badge>
            </div>
          </div>
          ...
          <div className="grid gap-6 p-6 sm:p-8 sm:grid-cols-[1.4fr_.9fr]">
            {/* Left: title + meta */}
            <div>
              <h1 className="mb-2 text-2xl sm:text-3xl font-bold tracking-tight text-white">{t.title}</h1>
              ...
              <div className="grid gap-2 text-sm sm:grid-cols-3">
                ...
                <Row icon={Users}>
                  {capacity ? `${registeredCount}/${capacity} slots` : `${registeredCount} registered`}
                </Row>
              ...
              {capacity > 0 && (
                <div className="mt-4">
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
                    <div className="h-full bg-indigo-500" style={{ width: `${filledPct}%` }} />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5">
              <div className="mb-3 text-sm text-white/70">Ready to compete?</div>
   ...
   />
          ...
          <div className="flex gap-2 overflow-x-auto rounded-xl border border-white/10 bg-white/5 p-1">
            {[
              ...
              <button
                ...
                ${activeTab === key ? 'bg-indigo-500 text-white' : 'text-white/80 hover:bg-white/10'}`}
              >
        ...
        {showRegForm && !isRegistered && (
          <div className="mt-6 rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-white/0 p-4 sm:p-6">
            <div className="mb-3 text-lg font-semibold text-white">Team Registration</div>
            <div className="grid gap-3 sm:grid-cols-3">
              <Field label="Team name" placeholder="e.g., NightRaiders" value={teamName} onChange={(e) => setTeamName(e.target.value)} />
              <Field label="Phone number" placeholder="10-digit phone" value={phone} onChange={(e) => setPhone(e.target.value)} />
              <Field label="Your real name" placeholder="Captain full name" value={realName} onChange={(e) => setRealName(e.target.value)} />
            </div>

            <div className="mt-4">
              <div className="mb-2 text-sm text-white/80">Players (1–4 required, 5 optional)</div>
              ...
              <div className="mt-3 text-xs text-white/60">
                Ensure IGN name & ID exactly match in-game to avoid disqualification.
              </div>
            </div>
