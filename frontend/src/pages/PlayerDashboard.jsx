...
import { Calendar, Users, Trophy, Save, Clock, X, Star, User as UserIcon, ArrowRight, ChevronRight } from 'lucide-react';
...
const PlayerDashboard = () => {
  ...
  const fetchRegisteredTournaments = async () => {
    try {
      ...
      const res = await tournamentsAPI.list({ participantId: meId, limit: 100 });
  ...
  const classifyScrim = (scrim) => {
    ...
    let end = scrim?.timeSlot?.end
      ...
      : (scrim?.timeSlot?.finish ? new Date(scrim.timeSlot.finish) : null);
  ...
  const stats = {
    totalScrims: upcomingScrims.length + ongoingScrims.length + completedScrims.length,
  ...
  const PaidBadge = ({ paid }) => (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium border
        ${paid ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-amber-500/10 border-amber-500/30 text-amber-300'}
      `}
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <span className="absolute -bottom-1 -right-1 h-5 w-5 rounded-full bg-emerald-500 grid place-content-center ring-2 ring-gray-900 text-[10px] font-bold">✓</span>
              </div>
              <div>
                <h1 className="text-2xl md:text-3xl font-bold">
                  Welcome back, <span className="text-gaming-purple">{user?.name || 'Player'}</span>
                </h1>
                <p className="text-gray-400 text-sm">Manage your scrims & tournaments at a glance</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                className={`btn-primary inline-flex items-center ${!upcomingTournaments.length && 'opacity-60 cursor-not-allowed'}`}
                onClick={openNextUpcomingTournament}
                disabled={!upcomingTournaments.length}
                title={upcomingTournaments.length ? 'Go to next tournament' : 'No upcoming tournaments'}
              >
              ...
              {['scrims','tournaments'].map((k) => {
                ...
                return (
                  <button
                    ...
                    ${active ? 'bg-gaming-purple/20 text-gaming-purple ring-1 ring-gaming-purple/40' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700/40'}`}
                  >
                ...
                {['upcoming','ongoing','completed'].map((k) => {
                  ...
                  return (
                    <button
                      ...
                      ${active ? 'bg-gaming-cyan/20 text-gaming-cyan ring-1 ring-gaming-cyan/40' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700/40'}`}
                    >
                      {label}
                    </button>
                  );
                ...
                {['upcoming','past'].map((k) => {
                  ...
                  return (
                    <button
                      ...
                      ${active ? 'bg-gaming-cyan/20 text-gaming-cyan ring-1 ring-gaming-cyan/40' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700/40'}`}
                    >
                      {k === 'upcoming' ? `Upcoming (${upcomingTournaments.length})` : `Past (${pastTournaments.length})`}
                    </button>
                  );
                })}
              </nav>
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(tTab === 'upcoming' ? upcomingTournaments : pastTournaments).map((t) => {
                ...
                return (
                  <div
                    key={t._id}
                    className={`card overflow-hidden relative transition hover:shadow-2xl hover:shadow-gaming-purple/10 ${isPast ? 'opacity-90' : ''}`}
                  >
                    {t.bannerUrl && (
                      <img
                        src={t.bannerUrl}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="font-semibold text-lg leading-snug">{t.title}</div>
                        <div className="text-xs text-gray-400 mt-1">
                          {fmtDateTime(t.startAt)}
                          {t.endAt && <> <span className="text-gray-600">→</span> {fmtDateTime(t.endAt)}</>}
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 text-xs rounded-full border
                        ${isPast ? 'border-gray-600 text-gray-400' : 'border-gaming-purple/40 text-gaming-purple bg-gaming-purple/10'}
                      `}>
            ...
            {(tTab === 'upcoming' ? upcomingTournaments : pastTournaments).length === 0 && (
              <div className="mt-6">
                <EmptyState
                  icon={Users}
                  title={tTab === 'upcoming' ? 'No upcoming tournaments' : 'No past tournaments'}
                  desc={tTab === 'upcoming'
                    ? 'Register for tournaments and they will appear here.'
                    : 'Once you complete tournaments, they’ll be shown here.'}
                  ctaLabel={tTab === 'upcoming' ? 'Explore Tournaments' : undefined}
            ...
            <form onSubmit={async (e) => {
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 mt-2 flex items-center gap-2">
                  <UserIcon className="h-3.5 w-3.5" /> Use a square image (min 128×128) for best results.
                </div>
              </div>
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button type="submit" className="flex-1 btn-primary inline-flex items-center justify-center">
