...
const Chip = ({ active, onClick, children }) => (
  <button
    ...
    className={`px-3.5 py-2 rounded-2xl text-sm font-medium transition-all
      ...
      ${active ? 'bg-white text-gray-900 shadow' : 'bg-white/5 text-white/80 hover:bg-white/10 border border-white/10'}`}
  >
    {children}
  </button>
);

const SoftDivider = () => <div className="hidden md:block w-px self-stretch bg-white/10" />;
  ...
  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
    ...
    <div className="flex gap-4 overflow-hidden">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="min-w-[260px] h-32 rounded-xl bg-white/10 animate-pulse" />
      ))}
    </div>
  </div>
);
...
const EmptyState = ({ onReset }) => (
  <div className="text-center py-16 rounded-2xl border border-white/10 bg-white/5">
    ...
    <p className="text-white/60 mt-1">Try a different term or clear some filters.</p>
    {onReset && (
      <button
        onClick={onReset}
...
const FeePill = ({ fee }) => {
  ...
  return (
    <span
      ...
      ${paid ? 'bg-cyan-500/10 text-cyan-300' : 'bg-emerald-500/10 text-emerald-300'}`}
...
const CapBar = ({ now = 0, max = 1 }) => {
  ...
  return (
    <div className="w-full bg-white/10 rounded-full h-1.5 overflow-hidden" aria-label={`Capacity ${now} of ${max}`}>
...
const ScrimList = () => {
  ...
  // Backend filters
  const [filters, setFilters] = useState({ q: '', game: '', platform: '', sort: 'rank', page: 1 });
  ...
  useEffect(() => { fetchScrims(); /* eslint-disable-next-line */ }, [filters.page, filters.game, filters.platform, filters.sort, filters.q]);
  ...
  // ----- Date helpers -----
  const toLocalKey = (d) => { const dt = new Date(d); return `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`; };
  const todayKey = toLocalKey(new Date());
  const getDateSrc = (s) => s?.timeSlot?.start ?? s?.date;
  const isToday = (s) => { const src = getDateSrc(s); return src ? toLocalKey(src) === todayKey : false; };
  const isPast = (s) => { const src = getDateSrc(s); return src ? toLocalKey(src) < todayKey : false; };
  const isPastScrimStrict = (scrim) => { const dt = scrim?.timeSlot?.start ? new Date(scrim.timeSlot.start) : new Date(scrim.date); return dt < new Date(); };

  const formatDateTime = (scrim) => {
    const dt = scrim?.timeSlot?.start ? new Date(scrim.timeSlot.start) : new Date(scrim.date);
    try { return dt.toLocaleString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }); }
    catch { return new Date(dt).toLocaleString(); }
  };

  const currentPlayers = (scrim) => Array.isArray(scrim?.participants) ? scrim.participants.length : scrim.currentPlayers ?? 0;
  ...
  const clientFiltered = scrims
    ...
    .filter((s) => {
      ...
      if (priceExact !== '' && !Number.isNaN(Number(priceExact))) return fee === Number(priceExact);
      return true;
    })
    .filter((s) => {
  ...
  const openPastForOrg = (group) => {
    ...
    const items = clientFiltered.filter((s) => {
      const creatorId = (typeof s.createdBy === 'object' && s.createdBy?._id) || s.createdBy || s.organizationId || s.orgId || null;
  ...
  return (
    <div className="min-h-screen bg-[#0b0b12] text-white">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-white/60 text-sm">Organizations with scrims scheduled for today</p>
            </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <Chip active={feeFilter==='all'} onClick={() => setFeeFilter('all')}>All Fees</Chip>
            <Chip active={feeFilter==='free'} onClick={() => setFeeFilter('free')}>Free (₹0)</Chip>
            <Chip active={feeFilter==='25'} onClick={() => setFeeFilter('25')}>0–₹25</Chip>
            <Chip active={feeFilter==='50'} onClick={() => setFeeFilter('50')}>₹25–₹50</Chip>
            <Chip active={feeFilter==='60+'} onClick={() => setFeeFilter('60+')}>₹60+</Chip>
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              />
              {selectedDate && (
                <button onClick={() => setSelectedDate('')} className="rounded-xl border border-white/10 bg-white/5 py-2 px-3 text-sm hover:bg-white/10">
                  Clear
                </button>
              )}
              ...
              />
              {priceExact && (
                <button onClick={() => setPriceExact('')} className="rounded-xl border border-white/10 bg-white/5 py-2 px-3 text-sm hover:bg-white/10">
                  Clear
                </button>
              )}
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          <div className="grid gap-6">{Array.from({ length: 4 }).map((_, i) => <SkeletonRow key={i} />)}</div>
        ) : groups.length > 0 ? (
          <div className="space-y-6 mb-10">
            {groups.map((g) => {
              ...
              return (
                <section key={g.orgId || g.orgName} aria-label={`${g.orgName} scrims`} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  {/* Org header */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full overflow-hidden grid place-items-center border border-white/10 bg-white/10">
                        {g.orgAvatar ? (
                          ...
                          <span className="text-lg font-bold">{g.orgName?.charAt(0)?.toUpperCase() || 'O'}</span>
                        )}
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-3 text-sm text-cyan-300/90">
                          {g.orgId && (
                            <Link to={`/organizations/${g.orgId}`} className="inline-flex items-center hover:text-cyan-200">
                              <Eye className="h-4 w-4 mr-1" /> View Org Profile
                            </Link>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="text-2xl font-bold text-fuchsia-300 leading-none">{g.scrims.length}</div>
                      ...
                      >
                        <Clock className="h-4 w-4" /> Past Scrims {pastCount > 0 ? `(${pastCount})` : ''}
                      </button>
                    </div>
                  </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between gap-2 mb-2">
                              <h4 className="text-base font-semibold line-clamp-2" title={scrim.title}>{scrim.title}</h4>
                              <FeePill fee={scrim.entryFee} />
                            </div>

                            <div className="flex items-center gap-2 mb-2">
                              <GameTag game={scrim.game} />
                              <span className="text-[11px] text-white/60">{scrim.platform}</span>
                            </div>

                            <div className="space-y-1 text-sm text-white/80 mb-3">
                              <div><Calendar className="inline w-4 h-4 mr-1" />{formatDateTime(scrim)}</div>
                              <div className="flex items-center gap-2">
                                <Users className="inline w-4 h-4" />
                                <span className="text-white/80 text-sm">{playersNow}/{playersMax} players</span>
                              </div>
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <button
                  ...
                  className={`h-9 w-9 rounded-xl text-sm font-medium
                    ${filters.page === i + 1 ? 'bg-white text-gray-900' : 'bg-white/5 hover:bg-white/10'}`}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full overflow-hidden grid place-items-center border border-white/10 bg-white/10">
                    {pastOrg?.avatar ? (
                      ...
                      <span className="text-sm font-bold">{pastOrg?.name?.charAt(0)?.toUpperCase() || 'O'}</span>
                    )}
                  </div>
                  <div>
                ...
                <button onClick={() => setPastOpen(false)} className="rounded-xl border border-white/10 bg-white/5 p-1.5 hover:bg-white/10" aria-label="Close">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="p-5 max-h-[70vh] overflow-y-auto">
                {pastList.length > 0 ? (
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {pastList.map((scrim) => (
                      <div key={scrim._id} className="rounded-xl border border-white/10 bg-white/5 p-4">
                        <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <div className="space-y-1 text-sm text-white/80 mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            <Users className="inline w-4 h-4" />
                            <span className="text-white/80 text-sm">{currentPlayers(scrim)}/{maxPlayers(scrim)} players</span>
                          </div>
                          <CapBar now={currentPlayers(scrim)} max={maxPlayers(scrim)} />
                        </div>
                        <Link
                          to={`/scrims/${scrim._id}`}
                  ...
                  <div className="text-center text-white/60 py-10">No old scrims found for this organization.</div>
                )}
              </div>
            </motion.div>
          </motion.div>
...
function groupByOrg(list) {
  ...
  for (const s of list) {
    ...
    if (!map.has(key)) {
      map.set(key, { orgId: orgId ? String(orgId) : null, orgName, orgAvatar, scrims: [] });
    }
